...
def scrape(server_url, RULE_ID, chapter_info):
    global try_times
    h, c = chapter_info
    ...
    if p == None and try_times > 0:
        ...
        try_times -= 1
        _, p = scrape(server_url, RULE_ID, chapter_info)
    if p == None:
        print("[err] downloaded %s failed. %s" % (c, h))
        p = ""
    else:
        print("downloaded %s" % c)
    return (c, p)
...
def download_novel(url_entry):
    ...
    func = partial(scrape, server_url, rule_id) #wrap multiple arguments
